...
export function ProductCard({ product, onQuickView, showSwatches = true, className }:
  ...
  return (
    <article className={cx('product-card group', className)}>
      ...
      <button className="product-card__wish icon-btn" data-active={wished}
              onClick={() => toggleWish(product.id)}
              aria-pressed={wished} aria-label={wished ? `Remove ${product.title} from wishlist` : `Save ${product.title}`}
      ...
      {onQuickView && (
        <div className="product-card__quickbuy">
          <button className="btn btn-overlay btn-sm btn-block" onClick={() => onQuickView(product)}>Quick view</button>
        </div>
      )}
